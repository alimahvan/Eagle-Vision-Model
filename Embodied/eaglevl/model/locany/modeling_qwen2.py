...
from .mask_magi_utils import build_magi_ranges, convert_mtp_mask_to_magi_plan
...
class Qwen2FlashAttention2(Qwen2Attention):
    """Qwen2 attention using the real FlashAttention2 kernels."""
...
class Qwen2MagiAttention(Qwen2Attention):
    ...
    def forward(
        ...
        if not isinstance(attention_mask, dict):
            raise ValueError("Qwen2MagiAttention expects a Magi range-plan dict attention mask.")
    ...
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        if config._attn_implementation == 'magi' and not _MAGI_AVAILABLE:
            if is_flash_attn_2_available():
                ...
                logger.warning_once(
                    'magi_attention and flash_attn are not available, falling back to sdpa'
                )
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def __init__(self, config: Qwen2Config):
        ...
        self.is_packing_mode = False  # Set to True in training script for stream packing
    ...
    def _create_attention_mask(
        ...
        sub_sample_lengths: Optional[torch.Tensor],
        is_packing_mode: bool,
        ...
        def _build_data_index() -> torch.Tensor:
            if sub_sample_lengths is None:
                return torch.zeros(1, seq_length, dtype=torch.long, device=device)
            data_index = torch.cat([
                torch.full((length.item(),), idx, dtype=torch.long, device=device)
                for idx, length in enumerate(sub_sample_lengths)
            ]).unsqueeze(0)
            if data_index.shape[-1] != seq_length:
                raise ValueError(
                    f"Packing data_index length mismatch: data_index={data_index.shape[-1]}, seq_length={seq_length}, "
        ...
        if self._attn_implementation == "magi":
            if is_packing_mode:
                return convert_mtp_mask_to_magi_plan(
            ...
            if input_ids is not None and input_ids.numel() > 0:
                ar_decode = ar_decode or input_ids[0, -1].item() != self.text_mask_token_id
            return build_magi_ranges(
        ...
        if self._attn_implementation == "flash_attention_2":
            if is_packing_mode:
                raise NotImplementedError(
                    "flash_attention_2 does not support stream packing MTP masks; use `magi` or `sdpa`."
                )
            if input_ids is not None and (input_ids == self.text_mask_token_id).any():
                raise NotImplementedError(
                    "flash_attention_2 does not support custom MTP block masks; use `magi` or `sdpa` for MTP training."
                )
            if attention_mask is not None and attention_mask.ndim == 4:
                raise ValueError(
                    "flash_attention_2 expects a 2D padding mask or None, but received a 4D custom mask."
                )
            return attention_mask
        
        # ===== SDPA / Eager Attention =====
        if is_packing_mode:
            from .mask_sdpa_utils import create_mtp_packing_mask_4d
            return create_mtp_packing_mask_4d(
                block_size=self.block_size,
                position_ids=position_ids,
                data_index=_build_data_index(),
                causal_attn=self.causal_attn,
    ...
    def forward(
        ...
        # Check if we're in stream packing mode
        is_packing_mode = self.is_packing_mode and sub_sample_lengths is not None
        
        # Generate attention mask based on implementation type
        attention_mask = self._create_attention_mask(
            input_ids=input_ids,
            position_ids=position_ids,
            inputs_embeds=inputs_embeds,
            sub_sample_lengths=sub_sample_lengths,
            is_packing_mode=is_packing_mode,
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        sub_sample_lengths: Optional[torch.Tensor] = None,
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if cache_position is None:
            current_len = inputs_embeds.shape[1] if inputs_embeds is not None and past_key_values is None else input_ids.shape[1]
            past_seen = 0
            if past_key_values is not None:
                if isinstance(past_key_values, Cache):
            ...
            device = input_ids.device if input_ids is not None else inputs_embeds.device
            cache_position = torch.arange(past_seen, past_seen + current_len, device=device)
